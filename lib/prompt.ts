...
export function buildQuestionPrompt(params: {
  ...
  const structure = deep
    ? `paragraphs는 6~7개, 각 항목은 {"label": 소제목(4~10자), "text": 본문(220~280자)} 형태.
총 합산 글자 수 약 1,500자(±150자).
...
3) 실제 행동·상황: 이 경향이 일상·연애에서 구체적으로 어떤 모습으로 나타나는지 (실생활 예시 포함).
4) 겉과 속의 차이: 세 체계가 다른 방향을 가리키는 지점이 있으면 "겉으로는 ~하지만 내면은 ~" 형태로. 같으면 이 경향이 더 강한 이유.
5) 시기·흐름: 언제 이 흐름이 강해지거나 변하는지 구체적인 월·시기로.
6) 지금 할 것: 상담자가 주도권을 가질 수 있는 행동 방향. "A보다 B가 유리합니다" 형태.${
    dual
      ? `\n7) 두 사람 비교: 두 명식의 일간 상생·상극·비화 관계를 분석에 포함. 나와 상대방의 행동 패턴이 관계에서 어떻게 부딪히거나 맞물리는지.`
      : ""
  }${
    isReunion
    ...
    : `paragraphs는 4~5개, 각 항목은 {"label": 소제목(4~10자), "text": 본문(180~240자)} 형태.
총 합산 글자 수 약 850자(±100자).

[문단 구성]
1) 핵심 결론: value를 바탕으로 결론부터.
2) 왜 그런지: 명식 요소를 풀이 안에 녹여서.
3) 실제 상황: 구체적 행동·상황으로 번역.
4) 지금 할 것: 행동 가능한 방향.`;
