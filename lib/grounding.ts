...
const STAR_LOVE: Record<string, { ming: string; bucho: string }> = {
  ...
  천부: { ming: "포용과 안정의 별이라 무리하게 벌이지 않고 있는 것을 지키는 편입니다", bucho: "안정적이고 살림을 잘 꾸리는 상대를 뜻합니다" },
  태음: { ming: "섬세한 감수성과 내향성의 별이라, 표현보다 분위기로 마음을 전합니다", bucho: "배려 깊고 섬세한 상대를 뜻하지만, 예민함이 갈등의 씨앗이 됩니다" },
  탐랑: { ming: "매력과 다재다능의 별이라 손대는 곳이 많은 대신 하나에 진득하기 어렵습니다", bucho: "매력적이고 사교적인 상대를 뜻합니다. 주변 이성이 많아 신경 쓰입니다" },
  거문: { ming: "말솜씨와 의심을 함께 주는 별이라, 대화가 무기이자 갈등의 원인입니다", bucho: "말이 잘 통하지만 말다툼도 잦은 상대를 뜻합니다" },
  천상: { ming: "신의와 조율의 별이라 사이에 서서 균형을 맞추려 애씁니다", bucho: "신뢰가 두텁고 배려하는 상대를 뜻합니다" },
  ...
  파군: { ming: "변화와 파격의 별이라 한자리에 오래 머무르지 않고 판을 자주 갈아엎습니다", bucho: "만남과 헤어짐이 반복되기 쉬운 인연을 뜻합니다" },
};

/** 12지지(자축인묘진사오미신유술해)별 기운 — 일지(사주)와 명궁 위치(자미두수) 양쪽에서 재사용.
 *  같은 오행이라도 지지가 다르면 결이 갈리도록, 오행보다 한 단계 더 세밀한 12분류로 쓴다. */
const BRANCH_TRAIT = [
  "남들 쉴 때 혼자 조용히 시작해 두는 기질",     // 자
  "느려도 끝까지 붙잡고 다지는 기질",           // 축
  "남보다 먼저 뛰어들고 보는 기질",             // 인
  "상황에 맞춰 유연하게 뻗어 나가는 기질",       // 묘
  "판 자체를 갈아엎어 버리는 기질",             // 진
  "재고 따지며 속까지 들여다보는 기질",          // 사
  "한번에 확 타오르는 기질",                   // 오
  "부드럽게 감싸서 정리해 내는 기질",           // 미
  "빠르게 판단하고 곧장 실행하는 기질",          // 신
  "끝까지 다듬어 완성도를 올리는 기질",          // 유
  "맡은 자리를 끝까지 지켜 내는 기질",          // 술
  "웬만한 건 다 품고 넘어가는 기질",            // 해
];
...
/** 별자리의 "삶 전반" 성향 — SIGN_LOVE는 연애 전용이라 평생 총론·커리어·재물·건강에서 쓰면
 *  "상대를 이상화하는 본질이 초년기에 드러난다" 같은 문장이 나온다. 같은 별자리를 연애가 아닌
 *  삶의 태도로 옮긴 판본. */
const SIGN_LIFE: Record<string, string> = {
  양자리: "먼저 부딪혀 보고 판단하는",
  황소자리: "한번 쥔 것을 좀처럼 놓지 않는",
  쌍둥이자리: "여러 갈래에 동시에 관심이 뻗는",
  게자리: "내 사람과 내 자리부터 챙기는",
  사자자리: "드러나는 자리에서 힘이 붙는",
  처녀자리: "빈틈을 먼저 찾아내 다듬는",
  천칭자리: "어느 한쪽으로 기울지 않으려는",
  전갈자리: "한번 파고들면 끝까지 캐는",
  사수자리: "울타리 밖으로 나가려는",
  염소자리: "오래 걸려도 결국 형태를 만들어내는",
  물병자리: "정해진 틀을 먼저 의심하는",
  물고기자리: "경계를 흐리며 스며드는",
};
...
export function grounding(c: Chart) {
  ...
  return {
    ...
    sunLife: SIGN_LIFE[c.sun] ?? "",
    moon: c.moon,
    moonLove: SIGN_LOVE[c.moon] ?? "",
    moonLife: SIGN_LIFE[c.moon] ?? "",
    asc: c.asc,
    ascLove: c.asc ? SIGN_LOVE[c.asc] ?? "" : null,
    ascLife: c.asc ? SIGN_LIFE[c.asc] ?? "" : null,
 ...
 *  예전엔 명궁용 일반 성향(STAR_LOVE.ming)을 그대로 끌어다 써서, 커리어 답변이 "기준이 뚜렷한
 *  분야에 강하다"인데 근거는 "판을 자주 갈아엎는다"처럼 서로 반박하는 일이 생겼다. */
const STAR_DOMAIN: Record<string, { career: string; wealth: string; health: string }> = {
  ...
  태양: { career: "드러나는 자리에서 힘이 붙어 대외·영업·교육 쪽이 맞습니다", wealth: "베풀며 버는 구조라 돈이 들어오는 만큼 나가는 폭도 큽니다", health: "에너지를 밖으로 많이 써서 번아웃이 오면 회복이 더딥니다" },
...
/** 궁(영역)에 맞춘 주성 해석 — 커리어/재물/건강 리포트가 명궁용 일반 성향을 끌어 쓰지 않게 한다. */
