 ...
 *  −4~+4가 19~91로 펴진다. */
const pillarScore = (c: AxisCtx, p: [number, number] | null) => {
  if (!p) return 55;
  let n = 0;
  for (const el of p) {
    if (el === c.useEl) n += 2;
    else if (el === c.helpEl) n += 1;
    else if (el === c.avoidEl) n -= 2;
  }
  return 55 + n * 9;
};
...
const AXIS_FN: Record<string, ((c: AxisCtx) => number)[]> = {
  ...
  "love-life": [
    (c) => gScale(temper(c.g.식상 + c.g.재성 * 0.6) * capacity(c)) + (c.sinsal.includes("도화") ? 6 : 0), // 사교성 — 감당하는 만큼만 밖으로 나간다
    (c) => gScale(temper(c.g.식상) * 1.3 * capacity(c)),                             // 감정표현
    (c) => 55 + (c.g.식상 + c.g.비겁 * 0.6 - c.g.인성 - c.g.관성 * 0.8) * 9,          // 독립성
    (c) => cScale(c.gods.filter((x) => x === "정관" || x === "정재").length, 45, 12) * (0.65 + 0.35 * capacity(c)) + (c.g.관성 + c.g.재성) * 3, // 신뢰감
    (c) => gScale(temper(c.g.관성) * 1.2 * (0.6 + 0.4 * capacity(c))),               // 책임감 — 관살이 과한데 감당 못 하면 책임이 아니라 눌림이다
  ],
  // 궁합 — 두 명식 대조 점수를 축마다 다른 각도로 쪼갠다
  "love-compatibility": [
    (c) => Math.round((c.pairScore ?? 55) * 0.9 + gScale(c.g.식상) * 0.1),           // 케미
    (c) => cScale(c.gods.filter((x) => x === "정관" || x === "정재").length, 42, 12) * (0.65 + 0.35 * capacity(c)) + (c.g.관성 + c.g.재성) * 3, // 신뢰
    (c) => gScale(temper(c.g.식상) * capacity(c) + c.g.인성 * 0.4),                  // 소통
  ...
  "love-reunion": [
    ...
    (c) => gScale(temper(c.g.식상) * 1.2 * capacity(c)),                             // 소통 가능성
  ...
  career: [
    (c) => gScale(temper(c.g.관성 + c.g.비겁 * 0.4) * capacity(c)),                  // 리더십 — 눌리는 것과 이끄는 것은 다르다
    (c) => gScale(temper(c.g.인성) * 1.2),                                           // 전문성
    (c) => gScale(temper(c.g.식상) * capacity(c)) + (c.sinsal.includes("역마") ? 6 : 0), // 적응력
  ...
  wealth: [
    ...
    (c) => cScale(c.gods.filter((x) => x === "편재").length, 40, 15) + (c.g.재성 * 4 + c.g.식상 * 4) * capacity(c), // 투자 감각
    (c) => Math.max(20, Math.min(96, 90 - Math.round(c.g.비겁 * 14))),                // 소비 관리(비겁이 크면 샌다)
    (c) => gScale((c.g.식상 + c.g.재성 * 0.5) * capacity(c)),                        // 수입 다각화
    (c) => Math.round(gScale(temper(c.g.재성)) * 0.6 + c.strength * 0.4),            // 재물 그릇
  ],
  health: [
...
const AXIS_DECILES: Record<string, number[][]> = {
  "love-life": [[45.5, 50.9, 55.1, 57.9, 60.1, 63.6, 66.7, 71.2, 78.3], [34.9, 40.8, 44.8, 49, 52.9, 56.9, 62.9, 69.8, 78.2], [26.7, 32.9, 39.3, 45.1, 50.1, 54.5, 59.6, 66, 74.7], [51.9, 56.2, 59.6, 64, 67, 70.2, 74.3, 79.5, 84], [35.1, 42.7, 48.9, 52.2, 55.8, 59.9, 64.9, 70.9, 76]],
  "love-compatibility": [[40, 43.9, 46.9, 51, 53.8, 56.9, 61.5, 66, 74.1], [48.9, 53.3, 57.2, 61, 64, 67.3, 71.5, 76.7, 81.1], [35.9, 42.8, 48.4, 52.9, 56.5, 59.4, 62.5, 67.6, 72.3], [42.5, 47.5, 49.5, 50.5, 52.5, 55, 58.5, 60, 62.5], [46.3, 53.7, 56.9, 59.9, 64.6, 68.6, 71.4, 75.1, 82.3]],
  "love-reunion": [[44.5, 49.5, 54, 58.8, 63.8, 67.5, 72.8, 78.3, 87.5], [34.5, 39.9, 43.7, 47.5, 51.2, 54.8, 60.4, 66.7, 74.5], [33.6, 39, 42.1, 46.1, 48.8, 51.3, 55.1, 57.6, 62.3], [52, 53.6, 56, 60, 64, 65.6, 68.8, 72, 80], [28.8, 34.2, 39.2, 43.8, 47.2, 51, 54.6, 58.8, 64.8]],
  "life-overview": [[28.8, 34.2, 39.2, 43.8, 47.2, 51, 54.6, 58.8, 64.8], [39.4, 45.1, 49.9, 53.4, 56.2, 59.2, 62.8, 66.1, 71.5], [47.8, 53.1, 57.3, 60.2, 62.1, 64.6, 66.9, 69.8, 72.9], [43.2, 46.2, 49.8, 53.9, 57.4, 60.8, 66, 71.9, 78.9], [39.3, 42.5, 45.3, 47.8, 50.3, 52, 55, 57.8, 60.5]],
  "career": [[39.9, 44.2, 47.7, 50.8, 55.3, 59, 64.2, 68.6, 75.4], [34.2, 40.8, 48, 52.2, 55.8, 66, 70.2, 75.6, 85], [34.6, 39.6, 42.8, 46.6, 49.8, 53.2, 56.7, 61.7, 69.1], [48.5, 53.4, 60, 61.8, 64.4, 68.1, 75.9, 79.2, 90.4], [37.8, 42.5, 44.5, 47.5, 50, 52.5, 54.5, 58, 61.8]],
  "wealth": [[48, 49.4, 52, 55.8, 63.4, 69, 70.8, 75.2, 85.6], [46, 48.9, 50.9, 54.6, 60.9, 64.4, 67.5, 70.4, 80.3], [54.5, 61.5, 65.7, 70.9, 74.1, 76, 81.6, 85.8, 88.6], [44.3, 48.5, 52.4, 55.4, 58.3, 61.4, 65, 71.6, 78.1], [39.2, 43.3, 45.8, 48, 50.4, 52.8, 54.9, 58, 61.7]],
...
/** 원점수 → 백분위 → 눈금(50~94).
 *
 *  백분위 자체는 손대지 않고 눈금만 옮긴다. 사람의 등수는 그대로다.
 *  하위 10%가 50, 상위 10%가 94에 오도록 잡았다. */
function toPercentileScale(cid: string, axis: number, raw: number): number {
  const d = AXIS_DECILES[cid]?.[axis];
  if (!d) return Math.max(50, Math.min(94, Math.round(raw)));
  let below = 0;
...
export function radarStats(ctx: Ctx): RadarStats {
  ...
  const axisCtx: AxisCtx = {
    ...
    avoidEl: A.avoidEl,
    pel: {
      년: [STEM_EL[A.pillars.년!.stem], BRANCH_EL[A.pillars.년!.branch]],
      월: [STEM_EL[A.pillars.월!.stem], BRANCH_EL[A.pillars.월!.branch]],
