...
function extraGrounds(
  ...
  const mkAngles = (T: string) => [
    ...
    `${T}에서 잘되는 일과 안 되는 일의 격차가 큽니다. ${dom}. 반대로 ${lack}. 능력이 고르게 퍼진 게 아니라 한쪽만 뾰족해서, 맞는 일은 남들 야근할 때 먼저 끝내놓고 안 맞는 일은 붙잡고 밤을 새도 진도가 안 나갑니다. 그래서 사람마다 평가가 극단적으로 갈립니다.`,
    ...
    `${T}에서 강점과 약점이 한 세트로 붙어 다닙니다. ${dom}. 그런데 그걸 세게 밀어붙일수록 ${lack}는 쪽에서 탈이 납니다. 성과가 가장 좋았던 시기 직후에 오히려 사고가 났던 적이 있다면 그 구조입니다. 잘될 때 브레이크를 밟는 게 본능적으로 안 되는 사람이라, 미리 정해 둔 기준으로 멈추는 장치가 필요합니다.`,
    yang
      ? `${T}에서는 몸이 먼저 나가고 설명은 나중입니다. 설명할 시간에 처리하는 게 빠르다고 판단하는 쪽이라, 결과가 좋으면 추진력으로 평가받지만 어긋나면 "왜 말도 없이 그랬어"가 됩니다. 능력보다 소통에서 점수를 잃는 유형이라, 결정 자체보다 한 줄 공유하는 습관이 훨씬 큰 차이를 만듭니다.`
    ...
    g.sunMoonAligned
      ? `${T}에서 표정이 곧 답입니다. 겉과 속의 온도가 거의 같아서 웃으면 진짜 좋은 거고 표정이 굳으면 진짜 불편한 겁니다. 상대 입장에서는 눈치 볼 필요가 없어 편한 사람인데, 본인 입장에서는 숨기고 싶을 때도 안 숨겨져 불리한 순간이 생깁니다. 협상이나 면접처럼 속을 감춰야 하는 자리에서는 따로 준비가 필요합니다.`
    ...
    missing
      ...
      : `${T}에서 극단으로 치우치는 일이 드뭅니다. 어떤 상황이 와도 최소한의 대응은 되는 쪽이라 크게 무너지지 않는데, 대신 크게 튀지도 않습니다. 그래서 안정적이라는 평가와 무난하다는 평가를 동시에 받습니다. 뾰족한 무기가 적은 게 유일한 약점이라, 승부처에서는 의식적으로 한쪽에 힘을 몰아줘야 결과가 납니다.`,
    ...
    g.timeKnown
      ...
      : `${T}에서 첫인상과 오래 겪은 뒤의 평가가 갈립니다. 짧게 보는 사람일수록 이 사람을 잘못 읽어서, 면접이나 첫 만남처럼 한 번에 판단받는 자리가 상대적으로 불리합니다. 반대로 시간이 쌓이는 관계에서는 평가가 계속 올라갑니다. 급하게 인정받으려 애쓰기보다 겪을 시간을 확보하는 쪽이 결과가 좋습니다.`,
    ...
    `${T}에서 혼자 다 해내려 할수록 손해를 봅니다. ${dom}. 그 힘은 확실한데, 안 되는 자리까지 스스로 메우려 들면 시간과 에너지가 배로 듭니다. 문제는 도움을 요청하는 걸 실력 부족으로 여기는 경향이 있다는 점입니다. 잘하는 것에 집중하고 나머지를 넘겼을 때 결과가 가장 좋았을 텐데, 정작 그 경험을 잘 기억하지 못합니다.`,
  ];
