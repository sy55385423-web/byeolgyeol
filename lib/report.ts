...
function extraGrounds(
  ...
  const mkAngles = (T: string) => [
    ...
    `${T}에서 강점과 약점이 한 세트로 붙어 다닙니다. ${dom}. 그런데 그걸 세게 밀어붙일수록 ${lack}는 쪽에서 탈이 납니다. ${love ? "가장 좋았던 시기 직후에 오히려 크게 어긋났던 적이 있다면 그 구조입니다" : "성과가 가장 좋았던 시기 직후에 오히려 사고가 났던 적이 있다면 그 구조입니다"}. 잘될 때 속도를 늦추는 게 본능적으로 안 되는 사람이라, 미리 정해 둔 선에서 멈추는 장치가 필요합니다.`,
    yang
